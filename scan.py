...
def new_handle_input():
  ...
  python scan.py 10.19.38.0/24 [-o nt] [-p 80,81-85,...]''')
    ...
    if len(args) < 1:
        print('*********************************************************')
        print('*         A simple  port scanner By: Andrew             *')
        print('*    https://github.com/dela1974       *')
        print('*********************************************************')
        parser.print_help()
        exit()
...
def lib_check_ms_17_010(addr, port):
    ...
    try:
        ...
        data = s.recv(1024)
        user_id = data[32:34]
        tree_connect_andx_request = b'\x00\x00\x00' + chr(58 + len(
            addr)).encode() + b'\xff\x53\x4d\x42\x75\x00\x00\x00\x00\x18\x01\x20\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x2f\x4b' + user_id + b'\xc5\x5e\x04\xff\x00\x00\x00\x00\x00\x01\x00\x1a\x00\x00\x5c\x5c' + addr.encode() + b'\x5c\x49\x50\x43\x24\x00\x3f\x3f\x3f\x3f\x3f\x00'
